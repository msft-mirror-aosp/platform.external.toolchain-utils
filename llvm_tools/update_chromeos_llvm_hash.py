...
def ChangeRepoManifest(git_hash: str, chroot_path: Path):
    """Change the repo internal manifest for llvm-project."""
    manifest_path = manifest_utils.get_chromeos_manifest_path(chroot_path)
    manifest_dir = manifest_utils.get_chromeos_manifest_path(chroot_path).parent
    branch_name = "update-llvm-project-" + git_hash
    commit_lines = textwrap.dedent(
        f"""
    ...
    try:
        manifest_utils.update_chromeos_manifest(
            git_hash,
            chroot_path,
        )
        subprocess.run(["git", "-C", manifest_dir, "add", manifest_path.name], check=True)
